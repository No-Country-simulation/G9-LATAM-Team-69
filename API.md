...
- Las **recomendaciones** se generan con reglas explicables sobre las variables de hábito
  (uso en punta, horas de alto consumo, panel solar).
- La **tarifa** de referencia está fijada en `0.75` $/kWh, según sugiere el reto.

## CORS

Los orígenes permitidos se configuran en `WebConfig.java`. En el despliegue actual, frontend y
API comparten dominio (mismo origen), por lo que el proxy (Caddy) elimina la cabecera `Origin`
antes de reenviar a la API. Para un frontend en dominio distinto, agregar su URL a `allowedOrigins`.
